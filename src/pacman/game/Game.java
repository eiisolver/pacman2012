...
public final class Game
{
	...
	private void _initGhosts()
	{
		...
		for(GHOST ghostType : GHOST.values())
			ghosts.put(ghostType,new Ghost(currentMaze.lairNodeIndex,0,
					(int)(ghostType.initialLairTime*(Math.pow(LAIR_REDUCTION,levelCount))),MOVE.NEUTRAL));
	}
	...
	{	
		...
		for(GHOST ghostType : GHOST.values())
			ghosts.put(ghostType,new Ghost(Integer.parseInt(values[index++]),Integer.parseInt(values[index++]),
